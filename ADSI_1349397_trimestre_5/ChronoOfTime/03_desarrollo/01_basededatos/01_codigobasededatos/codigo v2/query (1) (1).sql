...
create table cot.phase_p(
	name_phase varchar(100) comment 'Phase name'
);
alter table cot.phase_p add column project_code mediumint not null comment 'Project foreign key (project code)';
alter table cot.phase_p add primary key (name_phase, project_code);
alter table cot.phase_p add column state bool not null comment 'Status in which the phase (finish, start, etc...)';
alter table cot.phase_p add constraint fk_phase_project foreign key (project_code) references cot.project (code) on update cascade;
alter table cot.phase_p add index fk_phase_project_idx(project_code);
...
alter table cot.activity add constraint fk_activity_phase foreign key (phase_name_phase, phase_project_code) references cot.phase_p (name_phase, project_code) on update cascade;
...
alter table cot.program add column level_t_code varchar(45) not null comment 'Level trainig fiel foraing ( identification code of the level trainig)' ;
alter table cot.program add constraint fk_program_levelt foreign key (level_t_code) references cot.level_trainig (code) on update cascade;
alter table cot.program add index fk_program_levelt_idx(level_t_code);
...
alter table cot.trimester_has_learning_result add column t_wd_initials varchar(10) comment 'Initials of the day at trimester' ;
alter table cot.trimester_has_learning_result add column lr_code varchar(45) comment 'Number of learning result (foreing key)';
...
alter table cot.trimester_has_learning_result add constraint fk_thlr_lr foreign key (lr_code, competence_code, program_code,learning_edition) references cot.learning_result (code, code_competence,   code_program,edition) on update cascade;
alter table cot.trimester_has_learning_result add constraint fk_thlr_t foreign key (t_wd_initials, trimester_number,levelt_code) references cot.trimester (initials, number, code) on update cascade;
alter table cot.trimester_has_learning_result add primary key ( competence_code,levelt_code, lr_code, program_code,learning_edition, t_wd_initials, trimester_number);
alter table cot.trimester_has_learning_result add index fk_thlr_lr_idx ( program_code, competence_code, lr_code,levelt_code,learning_edition);
alter table cot.trimester_has_learning_result add index fk_thlr_t_idx (t_wd_initials, trimester_number);
...
alter table cot.number_group_has_trimester_has_learning_result add column number_lr varchar(45) comment 'Number of learning result(foreing key)';
...
alter table cot.number_group_has_trimester_has_learning_result add constraint fk_nghthlr_thlr foreign key (competence_code,levelt_code, number_lr, program_code,learningtri_edition,  initials_dw, number_trimester) references cot.trimester_has_learning_result (competence_code,levelt_code, lr_code, program_code,learning_edition,t_wd_initials, trimester_number) on update cascade;
...
alter table cot.number_group_has_trimester_has_learning_result add primary key (number_trimester, initials_dw, number_lr, program_code, competence_code, number_group,levelt_code,learningtri_edition);
alter table cot.number_group_has_trimester_has_learning_result add index fk_nghthlr_thlr_idx (number_lr, program_code, competence_code, initials_dw, number_trimester,levelt_code,learningtri_edition);
...
create table cot.person_has_rol(
	p_number_document int comment 'Document number identifying the person'
);
alter table cot.person_has_rol add column p_id_document varchar(10) comment 'Reference code that identifies the type of document';
alter table cot.person_has_rol add column id_rol varchar(45) comment 'Code that identifies the role';
alter table cot.person_has_rol add constraint fk_phr_person foreign key (p_number_document, p_id_document) references cot.person (number_document, document) on update cascade; 
alter table cot.person_has_rol add constraint fk_phr_rol foreign key (id_rol) references cot.rol (id_rol) on update cascade;
alter table cot.person_has_rol add primary key (p_number_document, p_id_document, id_rol);
alter table cot.person_has_rol add index fk_phr_person_idx (p_number_document, p_id_document);
...
alter table cot.instructor add column lu_tol varchar(100) not null comment 'Select part time or full time';
...
alter table cot.instructor add constraint fk_i_lu foreign key (lu_tol) references cot.link_up (type_link) on update cascade;
...
alter table cot.instructor add index fk_i_lu_idx (lu_tol);
...
create table cot.edition(
	id_edition int comment 'Version identifier code'
);
...
alter table cot.edition add primary key (id_edition, id_trimester);
...
alter table cot.disponibility add column wd_initials varchar(10) comment 'Initials that identify the day';
...
alter table cot.disponibility add constraint fk_d_wd foreign key (wd_initials) references cot.working_day (initials) on update cascade;
...
alter table cot.disponibility add primary key (start_date, number_document, document, wd_initials, name_dayi);
alter table cot.disponibility add index fk_d_i_idx (number_document, document);
alter table cot.disponibility add index fk_d_wd_idx (wd_initials);
...
alter table cot.timetable add column id_edition int comment 'Number id of Edition';
alter table cot.timetable add column number_ct varchar(25) comment 'Number of current term';
...
alter table cot.timetable add constraint fk_tt_e foreign key (id_edition, number_ct) references cot.edition (id_edition, id_trimester) on update cascade;
...
alter table cot.timetable add primary key (name_head, num_environment, number_group, number_tri, initials_dai,trim_code, name_dai, id_edition, number_ct, id_num_document, type_document);
...
alter table cot.timetable add index fk_tt_e_idx (id_edition, number_ct);
