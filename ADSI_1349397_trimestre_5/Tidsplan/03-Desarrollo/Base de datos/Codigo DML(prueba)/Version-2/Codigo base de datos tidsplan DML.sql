...
CREATE TABLE tidsplan.ficha (
	number_group VARCHAR(100) NOT NULL, 
	end_date DATE NOT NULL, route VARCHAR(15) NOT NULL,
	start_date DATE NOT NULL,
	name_state VARCHAR(15) NOT NULL,
	program_code VARCHAR(70) NOT NULL, 
	version VARCHAR(50) NOT NULL,
	PRIMARY KEY (number_group),
	constraint FK_group_name_state FOREIGN KEY (name_state) REFERENCES tidsplan.state_group(name_state)  ON UPDATE CASCADE,
	index FK_group_name_state(name_state),
    constraint fk_group_trp foreign key (program_code, version) references tidsplan.training_program (id_code, version)  ON UPDATE CASCADE,
    index fk_group_trp_idx (program_code, version)
 );
...
create table tidsplan.group_has_trimester (
    ...
    workingday_initials varchar (10) not null comment 'Se le asignan iniciales a la jornada (llave foranea)',
    level_formation varchar (50) not null comment 'Se le asigna un nivel de formacion ',    
	constraint fk_ght_group foreign key (number_group) references tidsplan.ficha(number_group)  ON UPDATE CASCADE,
...
CREATE TABLE tidsplan.schedule (
    ...
    level_formation VARCHAR(50) NOT NULL COMMENT 'Se le asigna un nivel de formacion',
    constraint fk_schedule_version foreign key (id_version, id_trimester_current) references tidsplan.version (id_version, id_trimester_current) on update cascade,
    index fk_schedule_version_idx (id_version, id_trimester_current),
    ...
    CONSTRAINT fk_schedule_ght FOREIGN KEY (number_group, name_trimester, working_day_initials, level_formation) REFERENCES tidsplan.group_has_trimester (number_group, name_trimester, workingday_initials, level_formation)  ON UPDATE CASCADE,
    INDEX fk_schedule_ght_idx (number_group, name_trimester, working_day_initials, level_formation),
    ...
    INDEX fk_schedule_instructor (document_number , document),
    PRIMARY KEY (start_hour , id_version , name_day , id_trimester_current , number_environment , name_headquartes , number_group , name_trimester , working_day_initials , document_number , document, level_formation)
);
...
insert into tidsplan.ficha (number_group, start_date, end_date, route, name_state, version, program_code) values ('1349397', '2017-01-23', '2018-11-23', '3123', 'En formacion', '102', '228106'   );
insert into tidsplan.ficha (number_group, start_date, end_date, route, name_state, version, program_code) values ('1234456', '2017-01-23', '2017-11-23', '3343', 'En formacion', '102', '228120'   );
...
insert into tidsplan.schedule (start_hour, end_hour, id_version, name_day, id_trimester_current, name_modality, number_environment, name_headquartes, number_group, name_trimester, working_day_initials, document_number, document, level_formation) values ('6:00:00', '12:00:00', 3, 'Lunes', 'T3', 'presencial', '300', 'Barrio Colombia', '1349397', 'trimestre-3', 'D', '80013833', 'CC', 'tecnologo' );
