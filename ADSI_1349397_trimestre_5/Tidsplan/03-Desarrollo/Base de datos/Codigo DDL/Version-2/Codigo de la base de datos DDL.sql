...
create table tidsplan.group_has_trimester (
    ...
    workingday_initials varchar (10) not null comment 'Se le asignan iniciales a la jornada (llave foranea)',
    level_formation varchar (50) not null comment 'Se le asigna un nivel de formacion ',    
	constraint fk_ght_group foreign key (number_group) references tidsplan.fichaficha(number_group)  ON UPDATE CASCADE,
...
CREATE TABLE tidsplan.schedule (
	...
	constraint fk_schedule_tc_idx foreign key (id_trimester_current) REFERENCES tidsplan.trimester_current (id_trimester) ON UPDATE CASCADE,
    index fk_schedule_tc_idx (id_trimester_current),
