...
public class PhaseProjectDaoImpl extends AbstractDAO implements PhaseProjectDao
{
	/** 
...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT name_phase, status_phase, project_code FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( name_phase, status_phase, project_code ) VALUES ( ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET name_phase = ?, status_phase = ?, project_code = ? WHERE name_phase = ? AND project_code = ?";
	...
	/** 
	 * Index of column status_phase
	 */
	protected static final int COLUMN_STATUS_PHASE = 2;
	...
	public PhaseProjectPk insert(PhaseProject dto) throws PhaseProjectDaoException
	{
		...
		try {
			...
			stmt.setString( index++, dto.getStatusPhase() );
			...
			_e.printStackTrace();
			throw new PhaseProjectDaoException( "Exception: " + _e.getMessage(), _e );
		}
		finally {
	...
	{
		PhaseProject ret[] = findByDynamicSelect( SQL_SELECT + " WHERE name_phase = ? AND project_code = ?", new Object[] { namePhase, projectCode } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE status_phase = ? ORDER BY status_phase", new Object[] { statusPhase } );
	}
	...
	/**
	 * Method 'PhaseProjectDaoImpl'
	 * 
	 */
	public PhaseProjectDaoImpl()
	{
	}
	 ...
	 * @param userConn
	 */
	public PhaseProjectDaoImpl(final java.sql.Connection userConn)
	{
	...
	{
		return "joycd.phase_project";
	}

	/** 
	 * Fetches a single row from the result set
	 */
	protected PhaseProject fetchSingleResult(ResultSet rs) throws SQLException
	{
		if (rs.next()) {
			PhaseProject dto = new PhaseProject();
			populateDto( dto, rs);
			return dto;
		} else {
			return null;
	 ...
	 */
	protected PhaseProject[] fetchMultiResults(ResultSet rs) throws SQLException
	{
		...
		PhaseProject ret[] = new PhaseProject[ resultList.size() ];
	 ...
	 */
	protected void populateDto(PhaseProject dto, ResultSet rs) throws SQLException
	{
		dto.setNamePhase( rs.getString( COLUMN_NAME_PHASE ) );
		dto.setStatusPhase( rs.getString( COLUMN_STATUS_PHASE ) );
	 ...
	 */
	protected void reset(PhaseProject dto)
	{
	}
