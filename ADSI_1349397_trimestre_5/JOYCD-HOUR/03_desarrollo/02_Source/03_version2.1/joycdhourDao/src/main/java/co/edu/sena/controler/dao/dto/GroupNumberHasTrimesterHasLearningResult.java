/*
 * This source file was generated by FireStorm/DAO.
 * 
 * If you purchase a full license for FireStorm/DAO you can customize this header file.
 * 
 * For more information please visit http://www.codefutures.com/products/firestorm
 */

package co.edu.sena.controler.dao.dto;

import co.edu.sena.controler.dao.dao.*;
import co.edu.sena.controler.dao.factory.*;
import co.edu.sena.controler.dao.exceptions.*;
import java.io.Serializable;
import java.util.*;

public class GroupNumberHasTrimesterHasLearningResult implements Serializable
{
	/** 
	 * This attribute maps to the column group_number in the group_number_has_trimester_has_learning_result table.
	 */
	protected String groupNumber;

	/** 
	 * This attribute maps to the column trimester_name in the group_number_has_trimester_has_learning_result table.
	 */
	protected String trimesterName;

	/** 
	 * This attribute maps to the column working_day_initials in the group_number_has_trimester_has_learning_result table.
	 */
	protected String workingDayInitials;

	/** 
	 * This attribute maps to the column learning_result_code in the group_number_has_trimester_has_learning_result table.
	 */
	protected String learningResultCode;

	/** 
	 * This attribute maps to the column program_code in the group_number_has_trimester_has_learning_result table.
	 */
	protected String programCode;

	/** 
	 * This attribute maps to the column competence_code in the group_number_has_trimester_has_learning_result table.
	 */
	protected String competenceCode;

	/** 
	 * This attribute maps to the column level_formation in the group_number_has_trimester_has_learning_result table.
	 */
	protected String levelFormation;

	/** 
	 * This attribute maps to the column version in the group_number_has_trimester_has_learning_result table.
	 */
	protected String version;

	/**
	 * Method 'GroupNumberHasTrimesterHasLearningResult'
	 * 
	 */
	public GroupNumberHasTrimesterHasLearningResult()
	{
	}

	/**
	 * Method 'getGroupNumber'
	 * 
	 * @return String
	 */
	public String getGroupNumber()
	{
		return groupNumber;
	}

	/**
	 * Method 'setGroupNumber'
	 * 
	 * @param groupNumber
	 */
	public void setGroupNumber(String groupNumber)
	{
		this.groupNumber = groupNumber;
	}

	/**
	 * Method 'getTrimesterName'
	 * 
	 * @return String
	 */
	public String getTrimesterName()
	{
		return trimesterName;
	}

	/**
	 * Method 'setTrimesterName'
	 * 
	 * @param trimesterName
	 */
	public void setTrimesterName(String trimesterName)
	{
		this.trimesterName = trimesterName;
	}

	/**
	 * Method 'getWorkingDayInitials'
	 * 
	 * @return String
	 */
	public String getWorkingDayInitials()
	{
		return workingDayInitials;
	}

	/**
	 * Method 'setWorkingDayInitials'
	 * 
	 * @param workingDayInitials
	 */
	public void setWorkingDayInitials(String workingDayInitials)
	{
		this.workingDayInitials = workingDayInitials;
	}

	/**
	 * Method 'getLearningResultCode'
	 * 
	 * @return String
	 */
	public String getLearningResultCode()
	{
		return learningResultCode;
	}

	/**
	 * Method 'setLearningResultCode'
	 * 
	 * @param learningResultCode
	 */
	public void setLearningResultCode(String learningResultCode)
	{
		this.learningResultCode = learningResultCode;
	}

	/**
	 * Method 'getProgramCode'
	 * 
	 * @return String
	 */
	public String getProgramCode()
	{
		return programCode;
	}

	/**
	 * Method 'setProgramCode'
	 * 
	 * @param programCode
	 */
	public void setProgramCode(String programCode)
	{
		this.programCode = programCode;
	}

	/**
	 * Method 'getCompetenceCode'
	 * 
	 * @return String
	 */
	public String getCompetenceCode()
	{
		return competenceCode;
	}

	/**
	 * Method 'setCompetenceCode'
	 * 
	 * @param competenceCode
	 */
	public void setCompetenceCode(String competenceCode)
	{
		this.competenceCode = competenceCode;
	}

	/**
	 * Method 'getLevelFormation'
	 * 
	 * @return String
	 */
	public String getLevelFormation()
	{
		return levelFormation;
	}

	/**
	 * Method 'setLevelFormation'
	 * 
	 * @param levelFormation
	 */
	public void setLevelFormation(String levelFormation)
	{
		this.levelFormation = levelFormation;
	}

	/**
	 * Method 'getVersion'
	 * 
	 * @return String
	 */
	public String getVersion()
	{
		return version;
	}

	/**
	 * Method 'setVersion'
	 * 
	 * @param version
	 */
	public void setVersion(String version)
	{
		this.version = version;
	}

	/**
	 * Method 'equals'
	 * 
	 * @param _other
	 * @return boolean
	 */
	public boolean equals(Object _other)
	{
		if (_other == null) {
			return false;
		}
		
		if (_other == this) {
			return true;
		}
		
		if (!(_other instanceof GroupNumberHasTrimesterHasLearningResult)) {
			return false;
		}
		
		final GroupNumberHasTrimesterHasLearningResult _cast = (GroupNumberHasTrimesterHasLearningResult) _other;
		if (groupNumber == null ? _cast.groupNumber != groupNumber : !groupNumber.equals( _cast.groupNumber )) {
			return false;
		}
		
		if (trimesterName == null ? _cast.trimesterName != trimesterName : !trimesterName.equals( _cast.trimesterName )) {
			return false;
		}
		
		if (workingDayInitials == null ? _cast.workingDayInitials != workingDayInitials : !workingDayInitials.equals( _cast.workingDayInitials )) {
			return false;
		}
		
		if (learningResultCode == null ? _cast.learningResultCode != learningResultCode : !learningResultCode.equals( _cast.learningResultCode )) {
			return false;
		}
		
		if (programCode == null ? _cast.programCode != programCode : !programCode.equals( _cast.programCode )) {
			return false;
		}
		
		if (competenceCode == null ? _cast.competenceCode != competenceCode : !competenceCode.equals( _cast.competenceCode )) {
			return false;
		}
		
		if (levelFormation == null ? _cast.levelFormation != levelFormation : !levelFormation.equals( _cast.levelFormation )) {
			return false;
		}
		
		if (version == null ? _cast.version != version : !version.equals( _cast.version )) {
			return false;
		}
		
		return true;
	}

	/**
	 * Method 'hashCode'
	 * 
	 * @return int
	 */
	public int hashCode()
	{
		int _hashCode = 0;
		if (groupNumber != null) {
			_hashCode = 29 * _hashCode + groupNumber.hashCode();
		}
		
		if (trimesterName != null) {
			_hashCode = 29 * _hashCode + trimesterName.hashCode();
		}
		
		if (workingDayInitials != null) {
			_hashCode = 29 * _hashCode + workingDayInitials.hashCode();
		}
		
		if (learningResultCode != null) {
			_hashCode = 29 * _hashCode + learningResultCode.hashCode();
		}
		
		if (programCode != null) {
			_hashCode = 29 * _hashCode + programCode.hashCode();
		}
		
		if (competenceCode != null) {
			_hashCode = 29 * _hashCode + competenceCode.hashCode();
		}
		
		if (levelFormation != null) {
			_hashCode = 29 * _hashCode + levelFormation.hashCode();
		}
		
		if (version != null) {
			_hashCode = 29 * _hashCode + version.hashCode();
		}
		
		return _hashCode;
	}

	/**
	 * Method 'createPk'
	 * 
	 * @return GroupNumberHasTrimesterHasLearningResultPk
	 */
	public GroupNumberHasTrimesterHasLearningResultPk createPk()
	{
		return new GroupNumberHasTrimesterHasLearningResultPk(groupNumber, trimesterName, workingDayInitials, levelFormation, learningResultCode, programCode, competenceCode, version);
	}

	/**
	 * Method 'toString'
	 * 
	 * @return String
	 */
	public String toString()
	{
		StringBuffer ret = new StringBuffer();
		ret.append( "co.edu.sena.controler.dao.dto.GroupNumberHasTrimesterHasLearningResult: " );
		ret.append( "groupNumber=" + groupNumber );
		ret.append( ", trimesterName=" + trimesterName );
		ret.append( ", workingDayInitials=" + workingDayInitials );
		ret.append( ", learningResultCode=" + learningResultCode );
		ret.append( ", programCode=" + programCode );
		ret.append( ", competenceCode=" + competenceCode );
		ret.append( ", levelFormation=" + levelFormation );
		ret.append( ", version=" + version );
		return ret.toString();
	}

}
