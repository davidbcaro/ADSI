/*
 * This source file was generated by FireStorm/DAO.
 * 
 * If you purchase a full license for FireStorm/DAO you can customize this header file.
 * 
 * For more information please visit http://www.codefutures.com/products/firestorm
 */

package co.edu.sena.controler.dao.dto;

import co.edu.sena.controler.dao.dao.*;
import co.edu.sena.controler.dao.factory.*;
import co.edu.sena.controler.dao.exceptions.*;
import java.io.Serializable;
import java.util.*;
import java.util.Date;

public class LogAudit implements Serializable
{
	/** 
	 * This attribute maps to the column log_audit in the log_audit table.
	 */
	protected long logAudit;

	/** 
	 * This attribute maps to the column level in the log_audit table.
	 */
	protected String level;

	/** 
	 * This attribute maps to the column log_name in the log_audit table.
	 */
	protected String logName;

	/** 
	 * This attribute maps to the column message in the log_audit table.
	 */
	protected String message;

	/** 
	 * This attribute maps to the column date in the log_audit table.
	 */
	protected Date date;

	/** 
	 * This attribute maps to the column user in the log_audit table.
	 */
	protected String user;

	/**
	 * Method 'LogAudit'
	 * 
	 */
	public LogAudit()
	{
	}

	/**
	 * Method 'getLogAudit'
	 * 
	 * @return long
	 */
	public long getLogAudit()
	{
		return logAudit;
	}

	/**
	 * Method 'setLogAudit'
	 * 
	 * @param logAudit
	 */
	public void setLogAudit(long logAudit)
	{
		this.logAudit = logAudit;
	}

	/**
	 * Method 'getLevel'
	 * 
	 * @return String
	 */
	public String getLevel()
	{
		return level;
	}

	/**
	 * Method 'setLevel'
	 * 
	 * @param level
	 */
	public void setLevel(String level)
	{
		this.level = level;
	}

	/**
	 * Method 'getLogName'
	 * 
	 * @return String
	 */
	public String getLogName()
	{
		return logName;
	}

	/**
	 * Method 'setLogName'
	 * 
	 * @param logName
	 */
	public void setLogName(String logName)
	{
		this.logName = logName;
	}

	/**
	 * Method 'getMessage'
	 * 
	 * @return String
	 */
	public String getMessage()
	{
		return message;
	}

	/**
	 * Method 'setMessage'
	 * 
	 * @param message
	 */
	public void setMessage(String message)
	{
		this.message = message;
	}

	/**
	 * Method 'getDate'
	 * 
	 * @return Date
	 */
	public Date getDate()
	{
		return date;
	}

	/**
	 * Method 'setDate'
	 * 
	 * @param date
	 */
	public void setDate(Date date)
	{
		this.date = date;
	}

	/**
	 * Method 'getUser'
	 * 
	 * @return String
	 */
	public String getUser()
	{
		return user;
	}

	/**
	 * Method 'setUser'
	 * 
	 * @param user
	 */
	public void setUser(String user)
	{
		this.user = user;
	}

	/**
	 * Method 'equals'
	 * 
	 * @param _other
	 * @return boolean
	 */
	public boolean equals(Object _other)
	{
		if (_other == null) {
			return false;
		}
		
		if (_other == this) {
			return true;
		}
		
		if (!(_other instanceof LogAudit)) {
			return false;
		}
		
		final LogAudit _cast = (LogAudit) _other;
		if (logAudit != _cast.logAudit) {
			return false;
		}
		
		if (level == null ? _cast.level != level : !level.equals( _cast.level )) {
			return false;
		}
		
		if (logName == null ? _cast.logName != logName : !logName.equals( _cast.logName )) {
			return false;
		}
		
		if (message == null ? _cast.message != message : !message.equals( _cast.message )) {
			return false;
		}
		
		if (date == null ? _cast.date != date : !date.equals( _cast.date )) {
			return false;
		}
		
		if (user == null ? _cast.user != user : !user.equals( _cast.user )) {
			return false;
		}
		
		return true;
	}

	/**
	 * Method 'hashCode'
	 * 
	 * @return int
	 */
	public int hashCode()
	{
		int _hashCode = 0;
		_hashCode = 29 * _hashCode + (int) (logAudit ^ (logAudit >>> 32));
		if (level != null) {
			_hashCode = 29 * _hashCode + level.hashCode();
		}
		
		if (logName != null) {
			_hashCode = 29 * _hashCode + logName.hashCode();
		}
		
		if (message != null) {
			_hashCode = 29 * _hashCode + message.hashCode();
		}
		
		if (date != null) {
			_hashCode = 29 * _hashCode + date.hashCode();
		}
		
		if (user != null) {
			_hashCode = 29 * _hashCode + user.hashCode();
		}
		
		return _hashCode;
	}

	/**
	 * Method 'createPk'
	 * 
	 * @return LogAuditPk
	 */
	public LogAuditPk createPk()
	{
		return new LogAuditPk(logAudit);
	}

	/**
	 * Method 'toString'
	 * 
	 * @return String
	 */
	public String toString()
	{
		StringBuffer ret = new StringBuffer();
		ret.append( "co.edu.sena.controler.dao.dto.LogAudit: " );
		ret.append( "logAudit=" + logAudit );
		ret.append( ", level=" + level );
		ret.append( ", logName=" + logName );
		ret.append( ", message=" + message );
		ret.append( ", date=" + date );
		ret.append( ", user=" + user );
		return ret.toString();
	}

}
