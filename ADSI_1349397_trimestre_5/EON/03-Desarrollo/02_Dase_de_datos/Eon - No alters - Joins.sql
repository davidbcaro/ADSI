...
eon.Headquarters h
join
eon.ambience a on(h.name_Headquarters = a.name_Headquarters)
join eon.Schedule s on(s.number_Ambience = a.number_Ambience 
and a.name_Headquarters = s.name_Headquarters);
...
s.end_Time
from
EON.Current_Quarter cq
join 
EON.Version v on(cq.idCurrent_Quarter = v.idCurrent_Quarter )
join (eon.Schedule s) 
on v.idCurrent_Quarter = s.idCurrent_Quarter 
and v.idversion = s.idversion;
...
eon.Document do
join
eon.Client cl on do.id_document = cl.id_document
join 
eon.Instructor i on(cl.document_Number = i.document_Number and cl.id_Document = i.id_Document) 
join 
eon.Schedule s on ( i.document_Number = s.document_Number and i.id_Document= s.id_Document);
...
eon.Day d
join
eon.Schedule s on d.name_Day = s.name_Day;
...
eon.Modality m
join 
eon.Schedule s on m.modality_Name = s.Modality_Name ;
...
eon.Schedule s 
where
(h.name_Headquarters = a.name_Headquarters and 
s.number_Ambience = a.number_Ambience 
and a.name_Headquarters = s.name_Headquarters);
...
from
EON.Current_Quarter cq ,
EON.Version v,eon.Schedule s
where
cq.idCurrent_Quarter = v.idCurrent_Quarter 
and v.idCurrent_Quarter = s.idCurrent_Quarter 
and v.idversion = s.idversion;
...
eon.Schedule s 
where 
do.id_document = cl.id_document and
cl.document_Number = i.document_Number and cl.id_Document = i.id_Document 
and i.document_Number = s.document_Number and i.id_Document= s.id_Document;
...
eon.Schedule s
where
m.modality_Name = s.Modality_Name ;
