/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package co.edu.sena.wariosoftjpa.controller.dao;

import co.edu.sena.wariosoftjpa.controller.dao.InterfaceDAO;
import co.edu.sena.wariosoftjpa.model.jpa.entities.Specialty;
import java.util.List;

/**
 *
 * @author 1349397
 * @param <T>
 */
public interface SpecialtyDAO <T extends Specialty> extends InterfaceDAO<T>{
    public List<T> findByNameSpecialty(String nameSpecialty);
    public List<T> findByState(boolean state);
    public List<T> findByLikeNameSpecialty(String nameSpecialty);
    public int updatePk(String llaveNueva, String llaveVieja);
    
}
